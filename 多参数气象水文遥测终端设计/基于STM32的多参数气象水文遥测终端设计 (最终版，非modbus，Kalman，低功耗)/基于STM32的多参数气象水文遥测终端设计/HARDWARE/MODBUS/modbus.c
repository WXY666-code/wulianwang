...
void Modbus_Init()
{
  modbus.myadd = 1; //从机设备地址为1
  modbus.timrun = 0;    //modbus定时器停止计算
	modbus.slave_add=0x90;//主机要匹配的从机地址
}
// Modbus 3号功能码函数
// Modbus 主机读取寄存器值
void Modbus_Func3()
...
//	printf("从机返回 %d 个寄存器数据：\r\n",count/2);
	...
	{
//		printf("Temp_Hbit= %d Temp_Lbit= %d \r\ntemp= %d\r\n",(int)modbus.rcbuf[3+i],(int)modbus.rcbuf[4+i],(int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256);
		
//		sprintf(str, "%d", (int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256);
		
		OLED_ShowNum(1,1,(int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256,5);
		
//		OLED_ShowString(1, 1, str);	//1行1列显示字符串HelloWorld!
		...
		{
			case 0:
				data.Ligth =  10*((int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256);
//			    OLED_ShowNum(1,1,data.Ligth,5);
			break;
			case 2:
				data.UVI =  (float)(0.1*((int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256));
//			    sprintf(str1, "%.1f", data.UVI);
//			    OLED_ShowString(1, 7, str1);
			break;
			case 4:
				data.Temp =  (float)(0.1*(((int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256))-40);
//			    sprintf(str1, "%.1f", data.Temp);
//			    OLED_ShowString(1, 11, str1);
			break;
			case 6:
				data.Humi =  (int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256;
//			    OLED_ShowNum(2,1,data.Humi,2);
			break;
			case 8:
				data.Wind_speed =  (0.1*((int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256));
//			    OLED_ShowNum(2,4,data.Wind_speed,2);
			break;
			case 10:
				data.Gust_speed =  (0.1*((int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256));
//			    OLED_ShowNum(2,7,data.Gust_speed,2);
			break;
			case 12:
				data.Wind_direction =  ((int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256);
//			    OLED_ShowNum(2,10,data.Wind_direction,2);
			break;
			case 14:
				data.Rainfall_M =  (float)(0.1*((int)modbus.rcbuf[4+i]+((int)modbus.rcbuf[3+i])*256));
//			    sprintf(str1, "%.1f", data.Rainfall_M);
//			    OLED_ShowString(2,12,str1);break;
			default: break;
