...
void user_main(void)
{
    ...
    if (ret != OPRT_OK) {
        PR_ERR("app_weather_init failed");
    } else {
        PR_INFO("Weather module initialized successfully");
        
        // Test weather functionality
        WEATHER_DATA_T weather;
        ret = app_weather_get_data(&weather);
        if (ret == OPRT_OK && weather.is_valid) {
            PR_INFO("Weather test: %s %d°C %d%%", 
                   weather.city, weather.temperature, weather.humidity);
        } else {
            PR_WARN("Weather test failed: ret=%d, valid=%d", ret, weather.is_valid);
        }
    }
#endif
