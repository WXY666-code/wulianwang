...
static void __app_display_status_tm_cb(TIMER_ID timer_id, void *arg)
{
    ...
    static uint32_t weather_status_cnt = 0;
#endif
    ...
    // Update weather status every 5 minutes
#if defined(ENABLE_WEATHER_FEATURE) && (ENABLE_WEATHER_FEATURE == 1)
    if ((weather_status_cnt * DISPLAY_STATUS_TM) >= 300000 || weather_status_cnt == 0) {
        WEATHER_DATA_T weather;
        if (app_weather_get_data(&weather) == OPRT_OK && weather.is_valid) {
            char weather_str[128] = {0};
            snprintf(weather_str, sizeof(weather_str), "%s %d°C", 
                    app_weather_get_icon(weather.condition),
                    weather.temperature);
#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
            app_display_send_msg(TY_DISPLAY_TP_STATUS, (uint8_t *)weather_str, strlen(weather_str));
#endif
        }
        weather_status_cnt = 0;
    }
    weather_status_cnt++;
#endif
}
