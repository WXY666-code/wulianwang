...
static TIMER_ID sg_network_monitor_timer = NULL;
static netmgr_status_e sg_last_network_status = NETMGR_LINK_DOWN;

/***********************************************************
***********************function define**********************
***********************************************************/

static bool __app_check_network_status(void)
{
    netmgr_status_e status = NETMGR_LINK_DOWN;
    netmgr_conn_get(NETCONN_AUTO, NETCONN_CMD_STATUS, &status);
    if (status == NETMGR_LINK_DOWN) {
        PR_WARN("Network is down");
...
static void __app_network_monitor_cb(TIMER_ID timer_id, void *arg)
{
    netmgr_status_e current_status = NETMGR_LINK_DOWN;
    netmgr_conn_get(NETCONN_AUTO, NETCONN_CMD_STATUS, &current_status);
    
    if (current_status != sg_last_network_status) {
        PR_INFO("Network status changed from %d to %d", sg_last_network_status, current_status);
        sg_last_network_status = current_status;
        
        if (current_status == NETMGR_LINK_DOWN) {
            PR_WARN("Network connection lost!");
        } else {
            PR_INFO("Network connection restored!");
        }
    }
}
...
static void __app_ai_audio_evt_inform_cb(AI_AUDIO_EVENT_E event, uint8_t *data, uint32_t len, void *arg)
{
    ...
    PR_DEBUG("AI Audio Event: %d, len: %d", event, len);

    switch (event) {
    case AI_AUDIO_EVT_HUMAN_ASR_TEXT: {
        if (len > 0 && data) {
            PR_INFO("Received ASR text: %.*s", len, data);
            
            // Check network status
            if (!__app_check_network_status()) {
                return;
            }
            ...
            app_weather_handle_voice_query((const char *)data, len);
#endif
        } else {
            PR_WARN("Received empty ASR text");
        }
    } break;
    ...
    case AI_AUDIO_EVT_NONE: {
        PR_DEBUG("AI Audio Event: NONE");
    } break;

    default:
        PR_DEBUG("Unhandled AI Audio Event: %d", event);
        // Check network status for unhandled events
        if (!__app_check_network_status()) {
            PR_WARN("Network is down during unhandled event");
        }
        break;
    }
...
static void __app_ai_audio_state_inform_cb(AI_AUDIO_STATE_E state)
{
    ...
    PR_INFO("AI Audio State changed to: %d", state);
    ...
    case AI_AUDIO_STATE_UPLOAD:
        PR_DEBUG("AI Audio State: UPLOAD - checking network status");
        // Check network status during upload
        if (!__app_check_network_status()) {
            PR_ERR("Network is down during upload state");
        }
        break;
...
OPERATE_RET app_chat_bot_init(void)
{
    ...
    PR_INFO("Initializing chat bot...");

#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
    app_display_init();
#endif

    // Check network status before initializing AI audio
    netmgr_status_e status = NETMGR_LINK_DOWN;
    netmgr_conn_get(NETCONN_AUTO, NETCONN_CMD_STATUS, &status);
    if (status == NETMGR_LINK_DOWN) {
        PR_WARN("Network is down during chat bot initialization");
    } else {
        PR_INFO("Network is up during chat bot initialization");
    }
    ...
    tal_sw_timer_create(__app_network_monitor_cb, NULL, &sg_network_monitor_timer);
    tal_sw_timer_start(sg_network_monitor_timer, 5000, TAL_TIMER_CYCLE); // Check every 5 seconds
